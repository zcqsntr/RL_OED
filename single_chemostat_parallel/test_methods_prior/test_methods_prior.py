...
for i in range(30):
    ...
    if not actions_from_agent: # use hardcoded actions from the optimiser
        ...
        retrn = env.get_reward(trajectory)
        print(retrn)
...
print('d scores', np.mean(all_D_scores), np.std(all_D_scores))
...
np.save(save_path +'/all_D_scores.npy', all_D_scores)
