...
if __name__ == '__main__':
    ...
    params = json.load(open( os.path.join(os.path.dirname(os.path.abspath(__file__)), 'params_LV.json')))
    ...
    args = y0, xdot_LV, param_guesses, actual_params, n_observed_variables, n_controlled_inputs, num_inputs, input_bounds, dt, control_interval_time,normaliser
    env = OED_env(*args)
    ...
    for episode in range(int(n_episodes//skip)):
        ...
        for e in range(0, N_control_intervals):
            ...
            outputs = env.map_parallel_step(np.array(actions).T, actual_params, continuous = True, Ds = Ds)
            next_states = []
